...
<body class="bg-gray-900 font-[Outfit] text-gray-100 antialiased h-screen flex items-center justify-center p-4 relative overflow-hidden">
    ...
    <div class="w-full max-w-md bg-gray-800/80 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden border border-gray-700 relative z-10">
        <div class="p-8">
            ...
            <form action="{{ url('/admin-login') }}" method="POST" class="space-y-6" novalidate>
                @csrf
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input type="text" id="email" name="email" placeholder="admin@quizmaster.com" value="{{ old('email') }}"
                            class="block w-full pl-10 pr-3 py-3 border border-gray-600 rounded-xl bg-gray-900/50 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent sm:text-sm transition-all shadow-inner @error('email') border-red-500 focus:ring-red-500 @enderror">
                    </div>
                    @error('email')
                    ...
                    <label for="password" class="text-sm font-medium text-gray-300 ml-1">Password</label>
                     <div class="relative group">
                        ...
                        <input type="password" id="password" name="password" placeholder="••••••••"
                            class="block w-full pl-10 pr-3 py-3 border border-gray-600 rounded-xl bg-gray-900/50 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent sm:text-sm transition-all shadow-inner @error('password') border-red-500 focus:ring-red-500 @enderror">
                    </div>
                    @error('password')
                       <p class="text-xs text-red-400 ml-1">{{ $message }}</p>
                    @enderror
        ...
        <div class="bg-gray-800/50 p-4 border-t border-gray-700 text-center">
            <p class="text-xs text-gray-500">&copy; {{ date('Y') }} Quiz Master Admin System</p>
        </div>
    </div>
